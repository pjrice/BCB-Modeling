...
The individual model-fitted parameters for the 2-back task are here:

```{r}
dvs <- read_csv("twoBack_parEstResults.csv", 
                col_types = cols(
                  .default = col_double(),
...
write_csv(connectome, file="ga_2b.csv")
save(fit, fit.cv, X, Y, order, I, G, file="ga_2b.RData")
```
...
ggplot(lasso_df, aes(x=lambda, y=error)) +
  ...
  xlab(expression(lambda)) +
  ylab("Cross-Validation Error") +
  ggtitle(expression(paste(bold("Cross Validation Error Across "), lambda, bold(" for the "),italic(d), bold(" Lasso Regression")))) +
  geom_vline(xintercept = lasso_df$lambda[lasso_df$error==min(lasso_df$error)], size=2, linetype='dashed') +
  theme_pander() +
  theme(legend.position="right")

#ggsave("2b_bll_CVerror.png", dpi=300)
```
...
parCorr = cor.test(wcomparison$Observed,wcomparison$Predicted)

parCorrVal = round(as.numeric(parCorr$estimate),2)
parCorrPval = round(as.numeric(parCorr$p.value),2)

if (parCorr$p.value >= 0.05) {
  annoText = paste('r = ',parCorrVal,'\np = ',parCorrPval)
} else if (parCorr$p.value < 0.05 & parCorr$p.value >= 0.01) {
  annoText = paste('r = ',parCorrVal,'\np < 0.05')
} else if (parCorr$p.value < 0.01 & parCorr$p.value >= 0.001) {
  annoText = paste('r = ',parCorrVal,'\np < 0.01')
} else {
  annoText = paste('r = ',parCorrVal,'\np < 0.001')
}
...
p <- ggplot(wcomparison, aes(y=Predicted, x=Observed, col=(Predicted-Observed)^2)) +
  ...
  annotate("text",x=0.7,y=0.3,label=annoText) +
  xlab(expression(paste("ACT-R model estimated decay rate ", italic(d)))) +
  ylab(expression(paste("Lasso predicted decay rate ", italic(d)))) +
  theme(legend.position = "left")
...
#ggsave("2b_bll_predVest_scatter.png", dpi=300)
...
write_csv(connectome, file="bll_2b.csv")
save(fit, fit.cv, X, Y, order, I, G, file="bll_2b.RData")
```
### Determining the number of brain regions and networks considered by the Lasso regression

From the list of connections that survived the Lasso, we want to determine the number of unique brain regions that are included in the connections, and the number of networks that those regions belong to.

```{r}
minLambda = fit.cv$lambda.min
numIncBetas = nrow(connectome)
numTotBetas = nrow(conn_betas)
pcntIncBetas = numIncBetas/numTotBetas

uniqueRegions = unique(unlist(strsplit(connectome$connection,split='-')))
numRegions = length(uniqueRegions)

uniqueNetworks = unique(power2011$NetworkName[power2011$ROI %in% uniqueRegions])
numNetworks = length(uniqueNetworks)
```

### Making a table of the Lasso results

```{r}
testDF = data.frame(Connection=connectome$network_names,
                    Beta=connectome$Beta)

#order dataframe by absolute value of Beta
testDF = testDF[order(abs(testDF$Beta),decreasing=TRUE),]

#split df in half because it's long
firstHalfEnd = ceiling(nrow(testDF)/2)
secondHalfStart = firstHalfEnd+1

fhDF = testDF[1:firstHalfEnd,]
shDF = testDF[secondHalfStart:nrow(testDF),]

#tab_df(fhDF,file='Z:\\gp\\ACTR-WM\\dissertation\\figures\\tables\\LassoBetas\\zb_ga\\firstHalf.html')
#tab_df(shDF,file='Z:\\gp\\ACTR-WM\\dissertation\\figures\\tables\\LassoBetas\\zb_ga\\secondHalf.html')

#tab_dfs(list(fhDF,shDF))
```
...
write_csv(connectome, file="ia_2b.csv")
save(fit, fit.cv, X, Y, order, I, G, file="ia_2b.RData")
```



## Neural Correlates of `:LF`

We can repeat the process for `:LF`, the latency factor $F$.
...
ggplot(lasso_df, aes(x=lambda, y=error)) +
  ...
  xlab(expression(lambda)) +
  ylab("Cross-Validation Error") +
  ggtitle(expression(paste(bold("Cross Validation Error Across "), lambda, bold(" for the "),italic(F), bold(" Lasso Regression")))) +
  geom_vline(xintercept = lasso_df$lambda[lasso_df$error==min(lasso_df$error)], size=2, linetype='dashed') +
  theme_pander() +
  theme(legend.position="right")

#ggsave("2b_lf_CVerror.png", dpi=300)
```
...
p <- ggplot(wcomparison, aes(y=Predicted, x=Observed, col=(Predicted-Observed)^2)) +
  ...
  annotate("text",x=3.0,y=1.25,label=annoText) +
  xlab(expression(paste("ACT-R model estimated decay rate ", italic(F)))) +
  ylab(expression(paste("Lasso predicted decay rate ", italic(F))))
...
#ggsave("2b_lf_predVest_scatter.png", dpi=300)
...
write_csv(connectome, file="lf_2b.csv")
save(fit, fit.cv, X, Y, order, I, G, file="lf_2b.RData")
```
...
### Lasso regression of HCP Fluid Cognition Composite score

```{r}

HCPbehDF = read.csv('HCP_beh.csv',header=TRUE)


subjects <- dir("data")[grep("sub-", dir("data"))]
keep <-  ! subjects %in% bto_parts
subjects = subjects[keep]
X <- R[keep,]


HCP_Gf = HCPbehDF %>% dplyr::select(Subject,CogFluidComp_Unadj) %>% mutate(Subject=paste0('sub-',Subject)) %>% filter(Subject %in% subjects)




Y <- HCP_Gf$CogFluidComp_Unadj

dependent <- as_tibble(data.frame(Gf=Y))

ggplot(dependent, aes(x=Gf)) +
  geom_histogram(bins=10, col="white", alpha=0.5) +
  #scale_fill_viridis(option = "plasma", discrete=T) +
  geom_vline(xintercept = mean(dependent$Gf),
             linetype = "dashed") +
  xlab(expression("Goal Activation"))+
  ylab("Number of Participants") +
  ggtitle("Distribution of Goal Activation") +
...
observed <-tibble(Subject = subjects, 
                  Param = Y, 
                  Condition = "Observed")

predicted <- tibble(Subject = subjects, 
                    Param = prediction, 
                    Condition = "Predicted")
...
ggplot(comparison, aes(x=reorder(Subject, Param), y=Param, col=Condition)) +
  ...
  scale_color_d3() +
  xlab("Participant")+
  ggtitle(expression(paste("Predicted vs. Observed Fluid Intelligence Measure Gf ", italic(F)))) +
  annotate("text", x = 20, y = 0.35,
           label=paste("r =", round(cor(Y, prediction), 3))) +
  theme_pander() +
  ylab(expression(paste("Latency Factor ", italic(F)))) +
...
p <- ggplot(wcomparison, aes(y=Predicted, x=Observed, col=(Predicted-Observed)^2)) +
  ...
  xlab("Observed Gf") +
  ylab("Predicted  Gf") +
  ggtitle(expression(paste("Predicted vs. Observed ", "Gf"))) 
...
#write_csv(connectome, file="Gf_allParts.csv")
#save(fit, fit.cv, X, Y, order, I, G, file="Gf_allParts.RData")
```
