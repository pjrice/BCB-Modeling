...
def runTask (ga, ia, lf, bll, stimTiming):
    ...
    # load the two-back ACT-R model
    actr.load_act_r_model("/home/pjrice/gp/ACTR-WM/code/models/nBack/nBack-model-main.lisp")
    #actr.load_act_r_model("Z:\\gp\\ACTR-WM\\code\\models\\nBack\\nBack-model-main.lisp")
    ...
    tempChunkRetrievals = [x[0] if x is not None else None for x in retrievalRespChunks]
...
def compute_rmse(ga, ia, lf, bll, stimTiming, correctChunks, correctResponses, subjResponses, targetTypes):
    ...
    if len(modelResponses) != 80:
        ...
        retrievalChunks = [x[2] for x in modelResponses]
    
        # calculate model/participant target, lure, and nonlure accuracy vectors
        [model_totAccVec,model_tarAccVec,model_lurAccVec,model_nlrAccVec] = compute_model_acc_combined(correctResponses,modelResponses,correctChunks,retrievalChunks,targetTypes) #need to get retrievalChunks in runTask()
...
def objective_func (x, stimTiming, correctChunks, correctResponses, subjResponses, targetTypes):
    ...
    if x[3] > 1:
        ...
        return(compute_rmse(x[0], # ga
                        ...
                        x[2], # lf
                        x[3], # bll
                        stimTiming, correctChunks, correctResponses, subjResponses, targetTypes))
    
def estPars_byPart (rootDir):
    """Estimates parameters for the two-back HCP n-back task ACT-R model on a per-participant basis.
    ...
    # load the device
    actr.load_act_r_code(rootDir+"code/models/nBack/twoBack-device.lisp")
    #actr.load_act_r_code(rootDir+"code\\models\\nBack\\twoBack-device.lisp")
    ...
    correctResponses = read_correct_responses(rootDir+"data/twoBack_correctResponses.csv")    
    targetTypes = [x[1] for x in correctResponses]    
    
    # create the stimulus timing for the two-back condition
    stimTiming = create_stim_timing()
    
    # get the list of participants from the behavioral two-back data directory 
    partList = os.listdir(rootDir+"data/beh/2b/")

    # establish initial values and bounds for the four parameters    
    init = [1, 1, 2.25, 0.5]
    ...
    for part in partList:
        ...
        # read this participant's responses for the two-back condition of the n-back task from the file
        subjResponses = read_participant_data(rootDir+'data/beh/2b/'+part+'/2bResp.csv')
        ...
        if (subj_tarAcc_sum < 12) or (subj_lurAcc_sum < 12) or (subj_nlrAcc_sum < 31):
...
def expandEstResults_bootstrap (results):
    ...
    actr.load_act_r_code("/home/pjrice/gp/ACTR-WM/code/models/nBack/twoBack-device.lisp")
    
    correctResponses = read_correct_responses("/home/pjrice/gp/ACTR-WM/data/twoBack_correctResponses.csv")
    ...
    for i in range(0,len(results['partID'])):
        ...
        subjResponses = read_participant_data('/home/pjrice/gp/ACTR-WM/data/beh/2b/'+part+'/2bResp.csv')
        [subj_totAccVec,subj_tarAccVec,subj_lurAccVec,subj_nlrAccVec] = compute_acc(correctResponses,subjResponses)
        subjRTs = np.array([x[1] for x in subjResponses], dtype=np.float)
        np.nan_to_num(subjRTs, copy=False, nan=np.nanmean(subjRTs))
        ...
        for ii in range(0,100):
            ...
            [model_totAccVec,model_tarAccVec,model_lurAccVec,model_nlrAccVec] = compute_model_acc_combined(correctResponses,modelResponses,twoBackChunks,retrievalChunks,targetTypes) #need to get retrievalChunks in runTask()
...
def dLOO_ModelPredictions (parNames,poppedParNames,parCombos,estPars,meanPars,subjRTs,subj_totAccVec,stimTiming,targetTypes,correctResponses,twoBackChunks,nRuns):
    ...
    for parCombo in parCombos:
        ...
        for ii in range(0,nRuns):
            ...
            tryNum = 0
            goodNumResponses = False
            
            while not goodNumResponses:
                
                if tryNum > 100:
                    sys.exit('Tried too many times to get this participant!')
                ...
                if numResponses == 80:
                    ...
                    #calculate model target, lure, and nonlure accuracy vectors
                    modelResponses4Acc = [[i[0],i[1],i[2],j] for i,j in zip(modelResponses,targetTypes)]
                    retrievalChunks = [x[2] for x in modelResponses]
                    [model_totAccVec,model_tarAccVec,model_lurAccVec,model_nlrAccVec] = compute_model_acc_combined(correctResponses,modelResponses4Acc,twoBackChunks,retrievalChunks,targetTypes)
                    temp_tbtAccs.append(model_totAccVec)
                    
                    goodNumResponses = True
                else:
                    tryNum = tryNum+1
                    print('Number of tries to get proper response length: '+str(tryNum))
...
def parameterVarExp_dLOO (estResults,nRuns=100):
    ...
    #parameter names
    parNames = ['bll','ga','ia','lf']
    parNames2pop = ['bll','ga','ia','lf']
