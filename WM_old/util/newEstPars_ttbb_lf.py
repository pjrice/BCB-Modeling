...
def runTask (css, ga, ia, lf, stimTiming):
...
def compute_rmse(css, ga, ia, lf, stimTiming, correctResponses, subjResponses, targetTypes):
    
    #run model, add targetTypes to model responses
    tempModelResponses = runTask(css, ga, ia, lf, stimTiming)
    ...
    #rescale mean RTs by dividing by max allowable RT (2 seconds)
    #RTs will now be on same scale as accuracy (bounded by (0,1))
    model_ttRTs = [[i,j] for i,j in zip(modelRTs,targetTypes)]
    subj_ttRTs = [[i,j] for i,j in zip(subjRTs,targetTypes)]
    
    model_tarRT = np.mean([x[0] for x in model_ttRTs if x[1]=='target'])/2
    model_lurRT = np.mean([x[0] for x in model_ttRTs if x[1]=='lure'])/2
    model_nlrRT = np.mean([x[0] for x in model_ttRTs if x[1]=='nonlure'])/2
    
    subj_tarRT = np.mean([x[0] for x in subj_ttRTs if x[1]=='target'])/2
    subj_lurRT = np.mean([x[0] for x in subj_ttRTs if x[1]=='lure'])/2
    subj_nlrRT = np.mean([x[0] for x in subj_ttRTs if x[1]=='nonlure'])/2
    
    #calculate model/participant block-by-block mean RTs
    #rescale mean RTs by dividing by max allowable RT (2 seconds)
    #RTs will now be on same scale as accuracy (bounded by (0,1))
    modelRT_byBlock = np.array([np.mean(x) for x in chunks(modelRTs,10)], dtype=np.float)/2
    subjRT_byBlock = np.array([np.mean(x) for x in chunks(subjRTs,10)], dtype=np.float)/2
    
    
    #create singleton vectors of combined scaled RTs and target/lure/nonlure accuracies for model and participant
    modelVec = np.concatenate((model_tarRT,model_lurRT,model_nlrRT,modelRT_byBlock,model_tarAcc,model_lurAcc,model_nlrAcc,modelAcc_byBlock), axis=None)
    subjVec = np.concatenate((subj_tarRT,subj_lurRT,subj_nlrRT,subjRT_byBlock,subj_tarAcc,subj_lurAcc,subj_nlrAcc,subjAcc_byBlock), axis=None)
...
def estPars_byPart (rootDataDir):
    ...
    init = [-0.5, 1, 1, 2.25]
    bnds = ((-1,-0.1), (0,4), (0,4), (1,3.5))
    
    results = results = dict(partID=list(), css=list(), ga=list(), ia=list(), lf = list(), converged=list())
...
#pickle.dump(test, open('/home/pjrice/gp/ACTR-WM/data/parEstResults/Powell/bounded/ttBBlf_parEstExp.p','wb'))
