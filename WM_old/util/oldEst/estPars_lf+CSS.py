...
def compute_rmse (lf, cueStimSim, stimTiming, correctResponses, subjResponses):
    
    #global stimTiming
    
    #run the model with the specified parameter values
    tempModelResponses = runTask(lf, cueStimSim)
...
def estPars_byPart (rootDataDir):
    ...
    results = dict(parts=list(), lfVals=list(), cssVals=list(), RMSEs=list(), modelAcc=list(), partAcc=list(), modelmRT=list(), partmRT=list())
    for part in partList:
        ...
        results['cssVals'].append(parEst.x[1])
...
def makeplots(results):
    ...
    n, bins, patches = plt.hist(results['RMSEs'])
    plt.xlabel('RMSE')
    plt.ylabel('Frequency')
    ...
    plt.ylabel('Frequency')
    plt.title('Model mean RT histogram')
    plt.show()
