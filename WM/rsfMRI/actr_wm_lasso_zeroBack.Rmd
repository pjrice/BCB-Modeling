...
ggplot(lasso_df, aes(x=lambda, y=error)) +
  ...
  ggtitle(expression(paste(bold("Cross Validation Error Across "), lambda, bold(" for the "),italic(W[g]), bold(" Lasso Regression")))) +
  geom_vline(xintercept = lasso_df$lambda[lasso_df$error==min(lasso_df$error)], size=2, linetype='dashed') +
  theme_pander() +
  theme(legend.position="right")

#ggsave("zb_ga_CVerror.png", dpi=300)
```
...
p <- ggplot(wcomparison, aes(y = Predicted, 
  ...
  annotate("text",x=1.5,y=0.25,label=annoText) +
  xlab(expression(paste("ACT-R model estimated decay rate ", italic(W[g])))) +
  ylab(expression(paste("Lasso predicted decay rate ", italic(W[g]))))
  #ggtitle(expression(paste("Predicted vs. Observed ", italic(W))))
...
#ggsave("zb_ga_predVest_scatter.png", dpi=300)
...
write_csv(connectome, file="ga_0b.csv")
save(fit, fit.cv, X, Y, order, I, G, file="ga_0b.RData")
```
...
write_csv(connectome, file="bll_0b.csv")
save(fit, fit.cv, X, Y, order, I, G, file="bll_0b.RData")
```
...
write_csv(connectome, file="ia_0b.csv")
save(fit, fit.cv, X, Y, order, I, G, file="ia_0b.RData")
```


## Neural Correlates of `:CSS`

We can repeat the process for `:CSS`, the cue-target similarity $S_{c,t}$.
...
write_csv(connectome, file="css_0b.csv")
save(fit, fit.cv, X, Y, order, I, G, file="css_0b.RData")
```
...
write_csv(connectome, file="lf_0b.csv")
save(fit, fit.cv, X, Y, order, I, G, file="lf_0b.RData")
```
