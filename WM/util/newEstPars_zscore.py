...
def compute_rmse(css, ga, ia, stimTiming, correctResponses, subjResponses, targetTypes):
    ...
    #zscore model/participant RTs/accuracies
    [subj_zRTs,model_zRTs] = zscore_wrt(subjRTs,modelRTs)
    [subj_zAcc,model_zAcc] = zscore_wrt(subj_totAccVec,model_totAccVec)
    
    #create singleton vectors of combined responses/RTs for model and participant
    subj_zResp = np.array([val for pair in zip(subj_zRTs,subj_zAcc) for val in pair], dtype=np.float)
    model_zResp = np.array([val for pair in zip(model_zRTs,model_zAcc) for val in pair], dtype=np.float)
    
    #compute rmse
    rmse = np.sqrt(np.mean((subj_zResp - model_zResp)**2))
...
pickle.dump(results, open('/home/pjrice/gp/ACTR-WM/data/zscore_parEst.p','wb'))
#results = pickle.load(open('/home/pjrice/gp/ACTR-WM/data/zscore_parEst.p','rb'))
