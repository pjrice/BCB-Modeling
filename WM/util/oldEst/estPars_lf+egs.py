...
def compute_rmse (lf, egs, stimTiming, correctResponses, subjResponses):
    
    #global stimTiming
    
    #run the model with the specified parameter values
    tempModelResponses = runTask(lf, egs)
...
def objective_func (x, stimTiming, correctResponses, subjResponses):
    return(compute_rmse(x[0], x[1], stimTiming, correctResponses, subjResponses))
...
#minimizeCompass(objective_func, x0=init, args=(stimTiming, correctResponses, subjResponses), bounds=bnds, deltatol=0.1, errorcontrol=False, paired=False, disp=True)
...
def estPars_byPart (rootDataDir):
    ...
    init = [0.29, 0.15]
    bnds = [(0.01, 1.5),(0.01,0.1)]
    
    actr.load_act_r_code("/home/pjrice/gp/ACTR-WM/code/devices/zeroBack-device.lisp")
    
    results = dict(parts=list(), lfVals=list(), egsVals=list(), RMSEs=list(), modelAcc=list(), partAcc=list(), modelmRT=list(), partmRT=list())
    for part in partList:
        ...
        #parEst = minimizeCompass(objective_func, x0=init, args=(stimTiming, correctResponses, subjResponses), bounds=bnds, deltatol=0.01, errorcontrol=True, paired=False, disp=True)

        
        
        tempModelResponses = runTask(parEst.x[0],parEst.x[1])
        tempResponses = [x[0] for x in tempModelResponses]
        tempRTs = [x[1] for x in tempModelResponses]
        #clean up responses/RTs into list of lists
        modelRespTimes = [round((i-j),3) if i is not None else i for i,j in zip(tempRTs,stimTiming.tolist())]
        ...
        results['egsVals'].append(parEst.x[1])
