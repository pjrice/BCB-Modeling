...
def compute_rmse (lf, stimTiming, correctResponses, subjResponses):
    ...
    modelRTs = np.array([round((i-j),3) if i is not None else i for i,j in zip(tempRTs,stimTiming.tolist())], dtype=np.float)
    ...
    np.nan_to_num(modelRTs,copy=False,nan=0)
    np.nan_to_num(subjRTs,copy=False,nan=0)
    
    #compute rmse
    RMSE = np.sqrt(np.mean((subjRTs-modelRTs)**2))
...
def makeplots(results):
    ...
    plt.title(':lf histogram - RT only')
    plt.show()
    ...
    plt.xlabel('RMSE')
    plt.ylabel('Frequency')
    plt.title('RMSE histogram - RT only')
    plt.show()
    ...
    plt.ylabel('Frequency')
    plt.title('Model mean RT histogram - RT only')
    plt.show()
    ...
    plt.ylabel('Frequency')
    plt.title('Difference between participant mean RTs and model mean RTs - RT only')
    plt.show()
